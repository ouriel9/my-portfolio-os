export const meta = {
  ...
  description: 'DESIGN-only audit of all Portfolio OS surfaces from comprehensive screenshots (every page, he/en × light/dark), per-surface experts → manager design work plan',
  phases: [
...
const SURFACES = [
  { key: 'streamlit-desktop', who: 'a senior UI designer reviewing the Streamlit app on DESKTOP', prefix: 'streamlit_desktop_', file: P.streamlit + ' (+ ' + P.streamlitCfg + ')', extra: `This is the priority surface. Scrutinize: sidebar layout/width, the tabbed bottom Settings, KPI cards, charts, dataframe tables, the dashboard sub-tabs, spacing/density on a wide screen, he vs en text fit, light vs dark.` },
  { key: 'streamlit-phone', who: 'a senior mobile UI designer reviewing the Streamlit app on PHONE', prefix: 'streamlit_phone_', file: P.streamlit + ' (+ ' + P.streamlitCfg + ')', extra: `Top priority + most problematic. Scrutinize: the collapsed sidebar/hamburger + overlay, bottom nav, RTL Hebrew, tap-target sizes, horizontal overflow, chart/table fit on a narrow screen, the loading bar, he vs en, light vs dark.` },
  { key: 'native-web', who: 'a senior desktop-app UI designer reviewing the native EXE web app', prefix: 'native_web_', file: P.nativeWeb + '/js/app.js + ' + P.nativeWeb + '/styles.css', extra: `Covers BOTH EXEs. Scrutinize: the sidebar nav, topbar, all 8 views (dashboard/holdings/allocation/reports/risk/simulator/agent/data), cards, charts, tables, the multi-theme picker, dark vs light theme correctness, empty states.` },
  { key: 'native-mobile', who: 'a senior mobile UI designer reviewing the native APK (mobile) app', prefix: 'native_mobile_', file: P.nativeMobile + '/app.js + ' + P.nativeMobile + '/styles.css', extra: `Covers the generic native APK. Scrutinize: bottom tabs, app bar, all views, cards/charts/tables on a phone, RTL Hebrew, the lock FAB placement, dark vs light theme, empty states.` },
  { key: 'cross-consistency', who: 'a design-systems reviewer checking CONSISTENCY + RTL + theming across ALL surfaces', prefix: '', file: 'all', extra: `Look across ALL screenshots (Streamlit desktop+phone + native web+mobile, both themes, he+en). Flag: inconsistent spacing/typography/color/branding between surfaces, RTL Hebrew correctness (LTR brand 'Portfolio OS' must not be force-RTL; numbers/dates direction), light-vs-dark theme bugs (a control that stays dark in light mode or vice-versa, low contrast, an un-themed element), and any element that renders well on one surface but poorly on another.` },
]
...
const plan = await agent(
  ...
  `streamlit-phone, native-web, native-mobile) based ONLY on the CONFIRMED defects — if a surface has no confirmed defects it is ` +
